...
class MLAFused(nn.Module):
    ...
    def __init__(self, model_config: ModelConfigMLA, dtype=torch.bfloat16):
        ...
        self.w_uk_uv = nn.Linear(model_config.kv_lora_rank, self.num_key_value_heads * (self.qk_nope_head_dim + self.v_head_dim), bias=False, dtype=dtype)

        self.w_dq = nn.Linear(model_config.dim, model_config.q_lora_rank, bias=False, dtype=dtype)
        self.w_qr_uq = nn.Linear(model_config.q_lora_rank, self.num_attention_heads * (self.qk_rope_head_dim + self.qk_nope_head_dim), bias=False, dtype=dtype)
    ...
    def forward(self, x_bsd, is_causal=False):
        ...
        q_r, q_n = torch.split(q_r_q_n, [self.num_attention_heads * self.qk_rope_head_dim, self.num_attention_heads * self.qk_nope_head_dim], dim=-1) # [B, S, num_attention_heads * qk_rope_head_dim], [B, S, num_attention_heads * qk_nope_head_dim]
